        ...
        >
          ...
          {(existingMedia.length > 0 || newFiles.length > 0) && (
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 10 }}>
              {existingMedia.map(m => (
                <MediaChip key={m.path} name={m.name || m.path.split("/").pop()} sizeMB={m.sizeMB} onRemove={() => removeExisting(m.path)} />
              ))}
              {newFiles.map((f, i) => (
                <MediaChip key={i} name={f.name} sizeMB={+(f.size / (1024 * 1024)).toFixed(1)} pending onRemove={() => removeNewFile(i)} />
              ))}
            </div>
          )}
        </Field>
...
function MediaChip({ name, sizeMB, pending, onRemove }) {
  return (
    <div style={{
      display: "flex", alignItems: "center", gap: 6, background: pending ? "#fff8e0" : SURFACE_ALT,
      border: `1px solid ${pending ? "#e0c568" : BORDER}`, borderRadius: 999, padding: "4px 6px 4px 12px", fontSize: 12.5,
    }}>
      ...
      <button onClick={onRemove} style={{ background: "none", border: "none", color: TERRACOTTA, cursor: "pointer", fontSize: 14, padding: "0 6px" }}>✕</button>
    </div>
  );
}
