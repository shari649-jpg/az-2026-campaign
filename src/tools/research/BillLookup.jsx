      ...
      {votesResult && (
        <div>
          <div style={{ background: B.surfaceAlt, borderRadius: '10px 10px 0 0', padding: '16px 20px', border: `1px solid ${B.border}`, borderBottom: 'none' }}>
            ...
            {votesResult.bill.sponsor && <p style={{ fontSize: 13, color: B.textMid, margin: '0 0 4px' }}>Sponsor: {votesResult.bill.sponsor.name}</p>}
            {votesResult.bill.latestAction && <p style={{ fontSize: 13, color: B.textMid, margin: 0 }}>Latest action ({votesResult.bill.latestAction.date}): {votesResult.bill.latestAction.text}</p>}
          ...
          <div style={{ border: `1px solid ${B.border}`, borderRadius: '0 0 10px 10px', padding: 20 }}>
            ...
            {votesResult.rollCalls.map((rc, i) => (
              <div key={i} style={{ marginBottom: 20, paddingBottom: 20, borderBottom: i < votesResult.rollCalls.length - 1 ? `1px solid ${B.border}` : 'none' }}>
                <p style={{ fontWeight: 700, fontSize: 14, marginBottom: 4 }}>
                  Roll Call #{rc.rollNumber} — {rc.voteQuestion || 'Vote'} {rc.result && <span style={{ color: rc.result === 'Passed' ? B.green : B.red }}>({rc.result})</span>}
                </p>
                <p style={{ fontSize: 12, color: B.textMute, marginBottom: 10 }}>{rc.date}</p>

                {rc.voteCoverageGap && (
                  <div style={{ background: '#fff7ed', border: '1px solid #f5c842', borderRadius: 8, padding: '10px 14px', marginBottom: 10, fontSize: 13, color: '#7a4f00' }}>
                    ...
                    <tbody>
                      {rc.candidateVotes.map((cv, j) => (
                        <tr key={j} style={{ borderTop: `1px solid ${B.border}` }}>
                          <td style={{ padding: '8px 10px' }}>{cv.name} <span style={{ color: B.textMute, fontSize: 12 }}>({cv.party}, {cv.district})</span></td>
                          <td style={{ padding: '8px 10px', textAlign: 'right' }}>
                            {cv.notInCongressForThisVote ? <span style={{ color: B.textMute, fontSize: 13 }}>Not in Congress for this vote</span> : <VoteBadge position={cv.position} />}
                          </td>
                        </tr>
                      ))}
                    </tbody>
