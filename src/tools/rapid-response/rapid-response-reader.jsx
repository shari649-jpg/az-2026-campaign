...
export default function RapidResponseReader() {
  ...
  const loadAll = async () => {
    try {
      const p = await window.storage.get("rr_profile");
      if (p) setProfile(JSON.parse(p.value));
    } catch {}
    try {
      const lib = await window.storage.get("rr_library");
...
Format:
{
  ...
  const pushToMachine = async () => {
    if (!article) return;
    // Save to shared storage slot that Message Machine reads
    const payload = {
    ...
    try {
      await window.storage.set("rr_pending_article", JSON.stringify(payload));
      setPushed(true);
