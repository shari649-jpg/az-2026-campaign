// netlify/functions/scheduled-community-notes.mjs
//
// Runs automatically every day at 6:00 AM UTC.
// Fetches X Community Notes TSV, parses + classifies, stores in Netlify Blobs.
// No Firebase. No extra credentials. Works on Netlify Pro out of the box.
//
// Modern Netlify Functions runtime (ESM) — schedule declared in config export,
// not via the legacy schedule() wrapper.
...
export default async function (req) {
  ...
  if (!notes.length) {
    console.error("[scheduled-community-notes] No political notes found.");
    return new Response("", { status: 200 });
  }
  ...
  try {
    ...
    console.log(`[scheduled-community-notes] ✓ Stored ${notes.length} notes in Netlify Blobs.`);
  } catch (err) {
    console.error("[scheduled-community-notes] Blob write failed:", err.message);
  }

  return new Response("", { status: 200 });
}

// Schedule declared here instead of the legacy schedule() wrapper
export const config = {
  schedule: "0 6 * * *",
};
