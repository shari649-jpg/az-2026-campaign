...
// This does NOT require admin/manager — any authenticated user may call it —
// it just stops the endpoint from being open to the public internet.
...
// Verifies the caller is a signed-in user. Throws on missing/invalid token.
...
export default async function (req) {
  ...
  try {
    ...
    console.error("[generate-message] admin init error:", err.message);
  ...
  // Accept the token either as an Authorization header or in the body,
  // matching the existing manage-user.mjs convention as a fallback.
